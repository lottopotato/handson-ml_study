...
def shuffle_dataset(dataset:dict):
	train_index = np.random.permutation(np.arange(train_num))
	test_index = np.random.permutation(np.arange(test_num))
	replaced_dataset = dataset

	for i in range(train_num):
		replaced_dataset['train_img'][i] = dataset['train_img'][train_index[i]]
		replaced_dataset['train_label'][i] = dataset['train_label'][train_index[i]]
	for j in range(test_num):
		replaced_dataset['test_img'][j] = dataset['test_img'][test_index[j]]
		replaced_dataset['test_label'][j] = dataset['test_label'][test_index[j]]
	return replaced_dataset


def load_mnist(normalize = True, flatten = False, one_hot = True, shuffle = False, \
	option_print = True):
	if not os.path.exists(save_file):
		init_mnist()
	with open(save_file, 'rb') as f:
		dataset = pickle.load(f)
		print("Reading from saved pickle..")
	...
	print(" - train-images shape : ", dataset['train_img'].shape)
	print(" - test-images shape : ", dataset['test_img'].shape)
	...
	if option_print:
		...
		print(" - one-hot coding : ", one_hot)
		print(" - shuffled : ", shuffle)
